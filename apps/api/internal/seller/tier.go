...
// (internal/order.MarkDelivered's Hous Trust instant-release branch).
type Tier string
...
const (
	...
	TierHousTrust Tier = "hous_trust"
)

// tierPct is the fixed commission ladder for every tier EXCEPT Hous
// Trust — Platinum is what Hous Trust used to be (500-order threshold,
// 5.50% rate) before Hous Trust became its own tier above it. Hous Trust
// deliberately has no entry here: its rate isn't fixed, it's negotiated
// per seller at application approval (see houstrust.go) depending on what
// they sell — pkg/fees.ComputeQuote's tierPct argument comes from
// PctForSeller below, never this map directly, specifically so a Hous
...
// housTrustFallbackPct is used only if a Hous Trust seller somehow has no
// custom_pct on file (should never happen — DecideHousTrustApplication
...
// Platinum's 5.50%, since Hous Trust is supposed to read as strictly the
// best tier even in this shouldn't-happen fallback case.
const housTrustFallbackPct = 0.0500

// PctForTier returns the FIXED commission percentage for t. Never call
// this for TierHousTrust — its rate isn't fixed, see PctForSeller. Kept
...
// sellerID given their current tier — every tier except Hous Trust uses
// the shared PctForTier ladder; Hous Trust's rate is individually
// negotiated per seller (depending on what they sell) at application
// approval, so it's read from users.hous_trust_custom_pct instead. This is
// the function every checkout/quote path must call — never PctForTier
// directly — precisely so a Hous Trust seller's actual negotiated rate is
...
func PctForSeller(ctx context.Context, pool *pgxpool.Pool, sellerID string, tier Tier) (float64, error) {
	if tier != TierHousTrust {
		return tierPct[tier], nil
	}
	var customPct *float64
	if err := pool.QueryRow(ctx, `select hous_trust_custom_pct from users where id = $1`, sellerID).Scan(&customPct); err != nil {
		return 0, fmt.Errorf("read hous trust custom pct: %w", err)
	}
	if customPct == nil {
		return housTrustFallbackPct, nil
	}
...
// daily worker) and houstrust.go's application flow; this function just
